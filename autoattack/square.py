...
class SquareAttack():
            ...
            if self.norm == 'Linf':
                ...
                for _ in range(h // s):
                    ...
                    for _ in range(w // s):
                        ...
                        vw += s
                    vh += s
                print('normalized_x', self.normalize(delta_init).shape, x.shape)
    ...
    def perturb(self, x, y=None):
        ...
        if y is None:
            if not self.targeted:
                with torch.no_grad():
                    output = self.predict(torch.cat((x.unsqueeze(1), self.aux_x), dim=1))
        ...
        if not self.targeted:
            acc = self.predict(torch.cat((x.unsqueeze(1), self.aux_x), dim=1)).max(1)[1] == y
        else:
            acc = self.predict(torch.cat((x.unsqueeze(1), self.aux_x), dim=1)).max(1)[1] != y
