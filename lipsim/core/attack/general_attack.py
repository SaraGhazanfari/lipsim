...
class GeneralAttack:
    def __init__(self, config):
        self.attack_names = {'AA': ['apgd-ce', 'square'], 'APGD': ['apgd-ce'], 'APGDT': ['apgd-t'],
                             'FAB': ['fab-t'], 'SQ': ['square']}
        self.config = config
    ...
    def generate_auto_attack(self, attack_method, attack_norm, img_0, img_1, img_ref, is_dist_attack, target,
        ...
        if is_dist_attack:
            ...
            img_ref = adversary.run_standard_evaluation(img_ref, target.long().unsqueeze(0), bs=img_ref.shape[
