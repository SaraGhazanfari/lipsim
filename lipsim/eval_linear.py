...
class LinearEvaluation:
    def __init__(self, config):
        ...
        self.linear_classifier = ClassificationLayer(self.config, embed_dim=N_CLASSES[self.config.teacher_model_name],
                                                     n_classes=1000)
        self.linear_classifier = self.linear_classifier.to(self.device)
        linear_classifier = nn.parallel.DistributedDataParallel(self.linear_classifier, device_ids=[self.config.gpu])
